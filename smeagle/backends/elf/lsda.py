...
class ExceptionTableHeader:
    ...
    def __init__(
        ...
        self.call_site_encoding = (
            call_site_encoding  # encoding of items in call site table
        )
        self.call_site_table_len = (
            call_site_table_len  # total length of call site table
        )
    ...
    def _parse_lsda_header(self):
        ...
        if lpstart_encoding != DW_EH_encoding_flags["DW_EH_PE_omit"]:
            ...
            lpstart = struct_parse(
                Struct("dummy", self._formats[base_encoding]("LPStart")), self.stream
            )["LPStart"]
        ...
        if ttype_encoding != DW_EH_encoding_flags["DW_EH_PE_omit"]:
            ttype_offset = struct_parse(
                Struct("dummy", self.entry_structs.Dwarf_uleb128("TType")), self.stream
            )["TType"]
        else:
            ttype_offset = None
        ...
        cstable_length = struct_parse(
            Struct("dummy", self.entry_structs.Dwarf_uleb128("CSTable")), self.stream
        )["CSTable"]
